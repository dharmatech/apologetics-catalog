...
## Display Modes and Per-Node Overrides

The browser should support different node display modes.

This is a view concern, not catalog data. The catalog entity remains unchanged.
The browser decides how much of that entity to render in a particular view.

Initial display presets:

* `minimal`: relationship label, entity label, kind badge, and perspective badge
* `summary`: `minimal` plus the main summary, proposition, or role
* `quote`: `minimal` plus quotation, source, and locator when available
* `full`: comprehensive inspection view with available modeled fields

The first version should prefer named presets over field-level checkboxes.
Field-level toggles may be added later if the presets prove too coarse.

There should be two default display profiles:

* `review`: richer defaults for inspection and model debugging
* `presentation`: denser defaults for walking someone through a reasoning chain
...
* assumptions: `summary`
* sources: `minimal`
* repeated nodes: `minimal`

Suggested `presentation` defaults:

* claims: `summary`
* evidence: `quote`
* arguments: `summary`
* interpretations: `minimal` or `summary`, depending on whether the
  interpretation itself is part of the presented chain
* assumptions: `minimal`
* sources: `minimal`
* repeated nodes: `minimal`

Per-node settings should be overrides on node instances.

An entity is a catalog record. A node instance is one rendering of that entity
at a particular place in a tree, snapshot, or presentation path. The same entity
may appear in several places. A display override should apply only to that
specific node instance unless the user explicitly chooses a broader operation.

Future controls may include:

* a settings button on each node
* preset selection for that node instance
* field-level checkboxes for that node instance
* apply this display mode to all occurrences of this entity in the current view
* apply this display mode to all nodes of this kind in the current view

Those broader operations should be explicit because they affect multiple node
instances.

Display settings belong to view state, presentation path state, or snapshot
state. They should not be written back into the canonical YAML records unless a
later authoring feature deliberately introduces reusable presentation documents.

---

## Presentation Paths

A presentation path is a user-curated ordered walkthrough of existing catalog
entities.

It is separate from canonical graph order.

Canonical graph order answers:

```text
What is connected to this node?
```

Presentation order answers:

```text
What sequence should I show someone?
```

Presentation paths should be optional artifacts layered over catalog data. The
catalog remains complete and browseable without them.

Multiple presentation paths may exist for the same root topic. Different paths
may target different audiences, levels of detail, or conversational goals.

The first implementation may keep presentation paths in browser state only.
Later versions may persist them as YAML or JSON documents.

Each presentation step should include:
...
steps:
  ...
  - entity_id: evidence.nwt_2013.colossians.1_15.firstborn
    relationship_id: relationship.jw.colossians_firstborn.supports_created
    display: quote
    lead_in: "See Colossians 1:15"
    level: 1

  - entity_id: argument.trinitarian.firstborn_not_necessarily_created
    display: summary
    lead_in: "Trinitarian response"
    level: 1
```

Relationship IDs should be stored when a presentation step follows a modeled
relationship. If no direct relationship exists, the step may still be included
as a manual sequence. Manual sequencing should be visible in inspection views so
the user can distinguish graph-modeled links from presentation-only ordering.

Nesting should be explicit. It should not be inferred only from graph
relationships. A user may want to flatten a rebuttal chain for readability or
nest a response under the point it answers.

Bridge notes should be presentation commentary. They are useful for transitions
such as:
...
Bridge notes are not catalog claims, arguments, evidence, or source quotations.
If a bridge note needs to become part of the modeled argument, it should be
promoted into normal catalog data.

Presentation paths should tolerate stale references gracefully.

If a referenced entity ID is missing, the viewer should render a broken
reference marker and continue rendering the rest of the path. If a relationship
ID is missing, the viewer should mark the step as manually sequenced or
relationship missing.

Exports should follow presentation order exactly, not canonical relationship
order. Broken references should be included in export warnings so shared output
does not silently hide missing data.

Later work may add:

* drag-and-drop step reordering
* indent and outdent controls
* add selected node to presentation path
* promote an expanded branch into a presentation path
* save and load named presentation paths
* ID alias or migration support for repaired references

---

## First Presentation View Milestone

The first presentation view should use browser-only state.

It should not persist presentation paths to YAML, JSON, local storage, or a
server. The goal is to test whether the ordering and display workflow feels
right before adding persistence.

Each presentation step should have a stable local step ID. UI controls should
target the step ID, not the step's array index, because moving steps changes
indexes.

Example first-pass browser state:

```js
{
  id: "step-1",
  entityId: "claim.jesus_created",
  relationshipId: null,
  level: 0,
  display: "summary"
}
...
* canonical browser remains the main workspace
* presentation view appears in a right-side panel
* the panel should not replace the tree view

Narrow-screen layout:

* presentation view appears as a bottom drawer or stacked panel
* controls remain button-based and touch-friendly

The presentation panel should initially have two tabs:

* `Outline`: editable ordered steps
* `Preview`: dense read-only rendering of the ordered presentation

First-pass interactions:

* add node to presentation from a node in the canonical browser
* move step up
* move step down
* indent step
* outdent step
* remove step
* clear all steps
* change display preset for a step
* view dense read-only preview
...
* bridge note editing
* drag-and-drop
* field-level display checkboxes
* saved named presentations

Bridge notes and exports are important, but they should wait until the core
outline and preview workflow is useful.
...
* expanded nodes
* relationship labels
* display profile and per-node display presets
* visible summaries, quotations, and locators
* perspective badges or labels
* repeated-node markers
* deterministic ordering
* presentation path ordering, when exporting a presentation path
* broken-reference warnings, when present
...
Snapshots should export the current visible state by default. If a node is in
`minimal`, `summary`, `quote`, or `full` mode, the exported representation
should honor that chosen mode. If an export format cannot preserve an
interactive setting, it should render the selected static form rather than
silently expanding to full detail.
